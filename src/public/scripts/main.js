...
(function () {
  ...
  function buildStars() {
    document.querySelectorAll("[data-stars]").forEach(function (field) {
      ...
      for (var i = 0; i < n; i++) {
        ...
        html += '<span class="star" style="left:' + (Math.random() * 100).toFixed(2) + "%;top:" +
          (Math.random() * 100).toFixed(2) + "%;width:" + size.toFixed(1) + "px;height:" + size.toFixed(1) +
          "px;--base:" + base + ";--peak:" + peak + ";--dur:" + (Math.random() * 4 + 3).toFixed(1) +
  ...
  function buildConstellation() {
    ...
    for (var i = 0; i < N; i++) {
      nodes.push({
        ...
        r: Math.random() * 1.6 + 1, big: Math.random() < 0.18
      });
    }
    ...
    function frame() {
      ...
      for (var i = 0; i < N; i++) {
        ...
        var ddx = mx - a.x, ddy = my - a.y, dd = Math.hypot(ddx, ddy);
        if (dd < 140) { a.x += ddx * 0.002; a.y += ddy * 0.002; }
        for (var j = i + 1; j < N; j++) {
          var b = nodes[j];
      ...
      for (var k = 0; k < N; k++) {
        ...
        if (n.big) { ctx.shadowColor = "oklch(0.8 0.11 228)"; ctx.shadowBlur = 10; }
  ...
  function contactForm() {
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (fields && ok) { fields.style.display = "none"; ok.classList.add("show"); }
    });
  }
